...
// Floating capsule in the frame-wide overlay layer (bottom-right). Polls the
// host route every 30 s. Collapsed: colored dot + headline percentage of the
// current provider's busiest window. Expanded: per-window progress bars with
// reset countdowns, plan tier, balance, and error states.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function pctColor(usedPct) {
      if (usedPct === undefined || usedPct === null) return '#8a8f98'
      if (usedPct < 60) return '#34a853'
      if (usedPct < 85) return '#f9ab00'
      return '#ea4335'
    }
    ...
    function Capsule() {
      ...
      const headline = windows.length > 0
        ? windows.reduce((a, b) => ((b.usedPct ?? -1) > (a.usedPct ?? -1) ? b : a), windows[0])
        : null
      ...
      const barBg = { height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.12)', overflow: 'hidden', marginTop: 4 }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() =>
        slots.inject('shell.overlay', () =>
          slots.register(
            { name: 'shell.overlay', id: 'dsh-quota-capsule', order: 90, label: 'Quota Capsule' },
            () => React.createElement(Capsule),
          )),
        'quota-capsule: overlay',
      )
    }
