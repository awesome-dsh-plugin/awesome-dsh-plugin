...
export const name = '@heiweilu/dsh-quota-capsule'
...
const ADAPTERS = [
  ...
  // ──────────────── 待接入适配器（调研笔记） ────────────────
  // Codex（ChatGPT 订阅）：
  //   官方 rate-limit 头（x-codex-*）只在对话响应里返回，无独立查询端点；
  //   可行路径是从 llm/stream 的响应头里采样限流字段，缓存后供本插件读取。
  // Claude（Anthropic 订阅/API）：
  //   响应头 anthropic-ratelimit-*-remaining / -reset 携带 5h 与日/周窗口；
  //   同样需要响应头采样，或接 Anthropic 的 rate-limit tokens API（若开放）。
  // MiMo（小米）：
  //   无公开配额端点文档；待确认 openapi 入口后补齐（key 变量名预留 MIMO_API_KEY）。
...
export function apply(ctx) {
  ...
  async function buildState(sessionId) {
    ...
    try {
      ...
      if (!Array.isArray(result.windows) || result.windows.length === 0) {
        // 供应商返回了 200 但没有可解析的窗口数据（响应结构变化 / 新套餐类型）
        return { ...base, ok: false, reason: 'empty-response', label: adapter.label }
      }
  ...
  async function state(sessionId) {
    ...
    // 错误结果不进缓存：排障时改完 key/网络能立即重试，不用等 30s 窗口过期
    const cacheable = cache.payload === null || cache.payload.ok !== false
    if (cacheable && cache.payload !== null && cache.key === cacheKey && Date.now() - cache.at < 30_000) return cache.payload
    const payload = await buildState(sessionId)
    if (payload.ok !== false) {
      cache.at = Date.now()
      cache.key = cacheKey
      cache.payload = payload
    } else if (cache.payload === null || cache.payload.ok !== false) {
      // 错误不覆盖缓存里的旧成功结果——UI 可以继续显示上次的成功数据
      cache.at = 0
      cache.key = null
    }
    return payload
