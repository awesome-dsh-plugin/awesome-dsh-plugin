...
- [tinqiao-oss/engramory](https://github.com/tinqiao-oss/engramory) - Curated file-based memory: a line/byte-capped `MEMORY.md` index plus one markdown file per fact, versioned with git and readable without a tool. The `dsh-engramory` plugin enforces that cap through `ctx.tools.guard()` and registers the protocol as a runtime skill; the same store is also read by Claude Code, Codex, Kiro, and OpenClaw.
### Tools & Capabilities
