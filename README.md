...
- [starslittle/dsh-blue-whale](https://github.com/starslittle/dsh-blue-whale) - Replicates DeepSeek Chat's own blue-whale palette (#4D6BFE); light and dark follow the built-in appearance.
...
- [JFWaskin/dsh-superpowers-bridge](https://github.com/JFWaskin/superpowers-safe/tree/fork/deepseek-harness-skills/fork/deepseek-harness-bridge) - Reuses the existing superpowers-safe `SKILL.md` files (~15 skills incl. safety-check) as a DSH `ctx.skills` provider via a Cordis plugin + `cordis.yml` overlay; lowest-friction install is one command, no `SKILL.md` rewrite needed. Ships a Claude-Code-shaped `hooks.json` for the SessionStart hook via `dsh-hooks-claude-code`.
### Workflow & Automation
...
- [luzhengyangtx/dsh-telegram-duty](https://github.com/luzhengyangtx/dsh-telegram-duty) - Telegram duty gateway: phone-message task loop on a dedicated duty session, global approval forwarding while on duty, duty/local toggle, zh/en messages.
