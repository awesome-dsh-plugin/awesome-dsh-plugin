...
- [reinocheong/dsh-session-move](https://github.com/reinocheong/dsh-session-move) - Move a session to another folder via drag & drop or a menu picker (reliable: header, log, projection cache and workspace accounting stay in sync), permanently delete with a risk-consent dialog, and AI-rename by summarizing the whole conversation (typo-aware). Each action also ships as an agent tool.
