...
- [keyiadiannao/dsh-restart-button](https://github.com/keyiadiannao/dsh-restart-button) — DSH 电源控制插件（独立实现）：侧边栏电源按钮 + 上拉「重启/关机」菜单 + Windows 关机风格全屏动画。自带重启/关机引擎，不依赖其它插件，动态端口适配，注册自己的 restart_harness 工具。
