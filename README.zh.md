...
- [xiajiajun516/dsh-config-manager](https://github.com/xiajiajun516/dsh-config-manager) — 一键备份 / 导出 / 导入 / 迁移整套 DSH 配置：设置、插件、MCP、技能、工作区。密钥永不导出、路径自动映射、导入先预览并自动备份支持回滚、支持多套配置档案，远程同步通过私有 Git 仓库推送可移植配置（快照恢复支持 CLI 与 GUI）。
