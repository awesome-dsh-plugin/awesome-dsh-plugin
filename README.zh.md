...
- [Oscar-Williams/dsh-deepatlas](https://github.com/Oscar-Williams/dsh-deepatlas) — 任务感知的插件导航:3000+ 插件本地索引,28 类中英能力分类检索;装前安全审计(风险信号+内容寻址缓存),事务化安装状态机(审计→同意→锁定 commit→组合/启动验证→ACTIVE,失败回滚)。
