...
- [AtinyFurina/dsh-memory-triage](https://github.com/AtinyFurina/dsh-memory-triage) — 记忆分诊：每轮自动判定保留/归档，并把条目路由到项目桶或总体桶；规则优先 + LLM 兜底（拿不准的条目每批 ≤8 条问模型），自动动作只归档不真删（/triage-run 全库分诊、/triage-purge 清理超期归档），零依赖。
