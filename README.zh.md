...
- [xiaomengxinbb/dsh-qq-bridge](https://github.com/xiaomengxinbb/dsh-qq-bridge) — 将 QQ（私聊与群聊 @）接入 DeepSeek Harness 的双向桥插件，每个 QQ 对话对应一个隔离、持久的 Agent 会话，支持命令体系、首访审批、附件入站（图片/语音/TXT/PDF）与本地文件出站发送。
