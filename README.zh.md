...
- [curtainsmall/dsh-electro-lab](https://github.com/curtainsmall/dsh-electro-lab) — DSH 电气电子计算插件：电路分析（阻抗、RC/RL/RLC 瞬态、谐振）、滤波器、匹配网络、信号质量、噪声与 dB 工具、DSP 变换、传输线与单位换算，全部基于 SI 基本单位的自描述复数值对象，每个数字都来自工具调用结果；附带 ElectroLab 模式智能体预设（隔离的计算沙箱）与记录面板，每次求解落盘存档、可供检视。
