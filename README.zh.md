...
- [windrover/dsh-long-term-memory](https://github.com/windrover/dsh-long-term-memory) — DSH 分层确定性长期记忆：CJK 感知 BM25 召回（无向量嵌入），JSONL 存储含用户画像/全局/工作区三作用域，每轮上下文注入，自动总结与 LLM/规则压缩，23 组威胁模式扫描，写入护栏，原子批量操作，可移植导出/导入，以及带记忆面板与设置卡片的 Web 界面。
