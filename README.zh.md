...
- [Junkrat9527/dsh-desktop-automation](https://github.com/Junkrat9527/dsh-desktop-automation) — macOS 桌面自动化插件：agent 工具直接控制鼠标/键盘/滚动/应用/窗口/截图；并整合视觉闭环（desktop_see 识图 + desktop_locate UI 定位坐标，复用 dsh 既有视觉凭据）。服务经 launchctl bootstrap gui 按需拉起（非登录自启）——因为只有 GUI 会话进程才能拿到 macOS 辅助功能权限。
