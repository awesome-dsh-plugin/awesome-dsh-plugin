...
- [THEWOLFWALKER/dsh-notifier](https://github.com/THEWOLFWALKER/dsh-notifier) — DSH 统一通知推送与远程控制：一个 `notify()` API 打通 25+ 渠道（Telegram / 钉钉 / 飞书 / 企业微信 / QQ 机器人 / WxPusher / PushPlus / Server 酱 / Bark / Discord / Slack / ntfy / webhook 等），按 timeSensitive / active / passive 分级路由并重试；五通道反向审批——在 Telegram 按钮、飞书交互卡片、QQ、WxPusher、微信（iLink）上直接批准 agent 请求；QQ/钉钉/飞书官方扫码登录（免手填凭证）；本地 Web 管理台（渠道健康、发送历史、审批收件箱、SSE 实时视图）；多 agent 路由矩阵（agent × 渠道绑定）；系统桌面通知；远程会话（followup / inject / steer）；回合结束/待审批/报错自动推送，agent 也可主动调用 notify 工具。密钥脱敏、工具限流、防打扰规则、零运行时依赖。
