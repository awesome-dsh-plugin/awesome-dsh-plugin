...
- [azure5100/huahua-dsh-record-replay](https://github.com/azure5100/huahua-dsh-record-replay) — 功能完整的 DSH 录制回放插件（Windows 完全支持）：回放会话时间线、导出/导入可分享回放包、复刻会话重新执行、录屏自动生成可安装技能。完整替代 Codex 录制回放，无 macOS 平台限制。
