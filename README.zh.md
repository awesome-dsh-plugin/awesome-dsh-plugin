...
- [linxuhao/Deepseek-Continuity](https://github.com/linxuhao/Deepseek-Continuity) — 本地生成图像/配音/音乐/音效，并锁定同一性：角色、动物、道具与配音角色定妆定声一次，之后每次调用复用同一份参考；退化产物（接近纯色的图、静音的音频）被拒绝而不是当成成功返回；空闲时引擎卸载显存。生图与听写也可改指向任意 OpenAI 形状的 API，那一半的本地引擎与权重就不装。
