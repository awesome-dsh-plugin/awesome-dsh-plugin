...
- [seven282/oss-prompt-optimizer](https://github.com/seven282/oss-prompt-optimizer) — 将一句随手写的话，经 harness llm 服务优化为专业、可直接使用的提示词——四段（Role / Task / Context / Format）或无标题纯文本风格。情境感知：自动解析指令与对话为角色/任务/目标画像，角色与任务强相关、目标与约束自动保留，并按「身份＋能力＋行为」三要素撰写、附任务类型写法建议；内置按任务类型与语言匹配的 few-shot 示例，简洁输出规则与按句断行让结果精炼可读；近失配热启动以相似缓存结果为起点迭代精修，/dream 造梦模式在结果末尾附明确标注的「AI 推断延伸洞察」（永不混入正文），快速档可在保质量前提下把耗时压到 3–5s。上下文感知默认开启；流式早停降低耗时。输入框 ✨ 一键优化/撤销/取消、prompt_optimize 工具（优化+迭代）、自动优化钩子与中英自动检测的角色文档。
