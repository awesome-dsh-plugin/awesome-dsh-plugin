...
- [ilps2/dsvu-videounderstanding](https://github.com/ilps2/dsvu-videounderstanding) — DeepSeek 驱动视频理解（DSV 版，dsh-video-understand 的衍生分支）：沿用 AVIS 信息层流水线，但主模型即视觉模型（deepseek-v4-flash-vision-exp）——关键帧与 ASR 信息层一次请求内完成（「同源看图作答」），网格拼图密集采样（≤384 token/图，输入成本低 10 倍），信息层前缀缓存（追问命中率 96.6%），官方峰谷定价（非高峰半价、周末无高峰）。3 视频 × 10 题盲测：质量 4.77 vs 3.83（MiMo 全量），成本约为原生视频输入的三分之一。
