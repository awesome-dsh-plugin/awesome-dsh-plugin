...
- [accpowered/dsh-auto-review](https://github.com/accpowered/dsh-auto-review) — 沙箱升级的自动审批应答器：确定性正则过滤先行，再由清洁上下文的审查模型裁决，不确定才问人；需应用仓库随附的核心补丁。
- [accpowered/dsh-credential-manager](https://github.com/accpowered/dsh-credential-manager) — 具名凭据管理器：模型按引用使用凭据，秘密值不进入对话，按次以 DSH_CM_* 环境变量注入 shell 执行，附设置页。
