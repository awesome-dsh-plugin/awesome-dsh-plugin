...
- [THEWOLFWALKER/dsh-notifier](https://github.com/THEWOLFWALKER/dsh-notifier) — 统一通知推送：一个 `notify()` API 打通 25+ 渠道，路由矩阵（时间敏感/主动/被动）+ 分级重试，多渠道入站审批（Telegram / 飞书卡片 / QQ / WxPusher / 微信 iLink），QQ/钉钉/飞书官方扫码登录，Web 管理台（渠道健康、发送历史、审批收件箱、SSE 实时视图），多 agent 路由与系统桌面通知；零运行时依赖。
