...
- [ ] I did not hand-edit the generated plugin-list blocks in the READMEs (local regeneration is optional; the outputs do not need to be committed) / 我没有手工修改 README 中由脚本生成的插件列表（本地生成仅用于可选预览，无需提交结果）
...
- [ ] `category` is one of `agi ui usage theme model identity session memory tools wsl browser vision voice docs skill workflow git notify dev security remote market fun`, and themes/skins go under `theme` / `category` 取值正确，主题/皮肤类请用 `theme`
